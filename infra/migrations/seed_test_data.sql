...
INSERT INTO users (id, phone, name, role, verification_level, created_at)
VALUES ('00000000-0000-0000-0000-000000000001', '+910000000001', 'Civitro Admin', 'admin', 'full', NOW())
...
-- Test polls (3) — use actual column names: question, type, created_by, ends_at
INSERT INTO polls (created_by, type, question, ends_at)
VALUES
  ('00000000-0000-0000-0000-000000000001', 'constituency', 'Should ward roads be repaved this quarter?', NOW() + INTERVAL '30 days'),
  ('00000000-0000-0000-0000-000000000001', 'budget', 'Best use of community hall funds (₹5 lakhs)?', NOW() + INTERVAL '30 days'),
  ('00000000-0000-0000-0000-000000000001', 'satisfaction', 'Rate the new garbage collection schedule', NOW() + INTERVAL '30 days')
ON CONFLICT DO NOTHING;

-- Poll options — use actual column: option_text
INSERT INTO poll_options (poll_id, label, votes_count)
SELECT p.id, opt.label, 0
FROM polls p
CROSS JOIN (VALUES
...
) AS opt(poll_question, option_text)
WHERE p.question = opt.poll_question
ON CONFLICT DO NOTHING;

-- Test voices (3)
INSERT INTO voices (user_id, text, likes_count, shares_count, location, created_at)
VALUES
  ('00000000-0000-0000-0000-000000000001', 'The new footpath near station road is excellent! Finally safe for pedestrians. Kudos to the ward team. #infrastructure #safety', 0, 0, ST_MakePoint(72.8777, 19.0760), NOW()),
  ('00000000-0000-0000-0000-000000000001', 'Water supply has been irregular for the past week in Sector 5. Can someone escalate this? #water #utilities', 0, 0, ST_MakePoint(72.8800, 19.0800), NOW() - INTERVAL '1 day'),
  ('00000000-0000-0000-0000-000000000001', 'Attended the gram sabha meeting today. Productive discussions on drainage. More citizens should participate! #democracy', 0, 0, ST_MakePoint(72.8750, 19.0700), NOW() - INTERVAL '2 days')
ON CONFLICT DO NOTHING;

-- Test promises (6, linked to representatives)
INSERT INTO promises (leader_id, promise_text, category, status, source, detected_at)
SELECT r.id, p.promise_text, p.category, p.status, p.source, p.detected_at
...
) AS p(designation, promise_text, category, status, source, detected_at)
