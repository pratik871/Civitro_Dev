...
import (
	...
	"github.com/civitro/services/community-action/internal/handler"
...
func main() {
	...
	// Initialize logger
	logger.Init()
	log := logger.WithService("community-action")

	log.Info().
		Int("http_port", 8022).
		Int("grpc_port", 50072).
		Msg("starting community-action service")
	...
	router.GET("/health", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "ok", "service": "community-action"})
	})
	...
	go func() {
		log.Info().Int("port", 8022).Msg("HTTP server listening")
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Fatal().Err(err).Msg("HTTP server failed")
		}
	}()

	// TODO: Start gRPC server on port 50072
	...
	log.Info().Msg("shutting down community-action service...")
	...
	log.Info().Msg("community-action service stopped")
}
