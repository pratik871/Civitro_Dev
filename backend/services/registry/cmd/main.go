...
import (
	...
	"github.com/civitro/services/registry/internal/handler"
...
func main() {
	...
	// Initialize logger
	logger.Init()
	log := logger.WithService("registry")
	...
	router.GET("/health", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "ok", "service": "registry"})
	})
	...
	go func() {
		log.Info().Int("port", 8003).Msg("HTTP server listening")
	...
	log.Info().Msg("shutting down registry service...")
	...
	log.Info().Msg("registry service stopped")
}
